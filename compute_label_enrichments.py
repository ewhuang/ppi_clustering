...
def get_label_dct():
    ...
    if label_type == 'dbgap':
        fname = 'dbgap_dct'
    with open('./data/%s_data/%s.json' % (species, fname), 'r') as fp:
...
def main():
    if len(sys.argv) != 4:
        print ('Usage:python %s <species> <n_clusters> <annotation_set>' %
            sys.argv[0])
        exit()
    global species, label_type, n_clusters
    ...
    folder = './results/%s_results' % species    

    for net_type in ['msigdb', 'none']:
        # Sort out file name.
        if net_type == 'msigdb':
            clusname = '%s/%s_clusters_msigdb_clean.txt' % (folder, n_clusters)
        elif net_type == 'none':
            clusname = '%s/%s_clusters_none.txt' % (folder, n_clusters)
        outname = '%s/%s_clusters_%s_terms_%s.txt' % (folder, n_clusters,
            label_type, net_type)
    ...
    #             folder, network, network, run_num)
    #     else:
    #         cluster_fname = '%s/clusters_%s/clusters_%s_%s.txt' % (
    #             folder, network, network, run_num)
                    
    #     subfolder = '%s/%s_enrichment_terms_%s' % (folder, label_type,
    #         network)
    ...
    #         label_type, network, run_num))
        compute_label_enrichments(clusname, outname)
